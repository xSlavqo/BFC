...
class OcrManager:
    ...
    def __init__(self, bot):
        ...
        self.reader = easyocr.Reader(['en', 'pl'], gpu=False)
        self.spell_en = SpellChecker(language='en')
        self.spell_pl = SpellChecker(language='pl')
        self.text_cache = {}
        self.logger.warning("Menedżer OCR z pamięcią podręczną i opcją klikania został zainicjowany.")
    ...
    def _find_word_in_results(self, results, target_word):
        for (bbox, text, prob) in results:
            ...
            if self.spell_pl.correction(found_word) == target_word:
                return bbox
        return None

    def find_text(self, text_to_find, click=False):
        """
        Robi zrzut ekranu i szuka na nim tekstu.
        ...
        if found_coords:
            ...
            if click:
                ...
                mouse_click(found_coords[0], found_coords[1])
            return found_coords
