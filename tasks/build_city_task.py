...
import time
# import pyautogui # Usunięto, bo akcje będą zdalne
...
class BuildCityTask(BaseTask):
    ...
    def _enter_main_building_menu(self):
        ...
        # Przekazujemy self.bot do mouse_click
        mouse_click(self.bot, th_x, th_y)
        time.sleep(2)

        # locate już używa self.bot
        if not locate(self.bot, "png/build/enter_upgrade_menu", perform_click=True):
    ...
    def _find_and_start_task(self):
        ...
        for i in range(max_search_loops):
            ...
            # Priorytet 1: Sprawdź, czy można coś ulepszyć od razu
            if locate(self.bot, r"png/build/upgrade.png", perform_click=True):
                self.logger.warning("Znaleziono i rozpoczęto ulepszanie budynku.")
                time.sleep(1)
                
                if i == 0:
                    # Jeśli to pierwsza iteracja, kliknij w ratusz
                    self.logger.warning("Pierwsze ulepszenie, klikanie w ratusz.")
                    # Przekazujemy self.bot do mouse_click
                    mouse_click(self.bot, *self.town_hall_coords)
                else:
                    # W kolejnych iteracjach, kliknij w środek ekranu
                    self.logger.warning("Kolejne ulepszenie, klikanie w środek ekranu.")
                    # Pobieramy rozmiar ekranu zdalnie
            ...
            # Jeśli nie ma ulepszenia, przejdź do następnego budynku
            if not locate(self.bot, r"png/build/go_button", perform_click=True, region=(691, 610, 1103, 658)):
                self.logger.error("Nie znaleziono przycisku 'Go', aby przejść do następnego budynku.")
                return False

            time.sleep(2)

            # Po kliknięciu 'Go', sprawdź czy można wejść do menu ulepszeń...
            if locate(self.bot, r"png/build/enter_upgrade_menu", perform_click=True):
                # Jeśli tak, kontynuuj pętlę, aby na nowo sprawdzić przycisk 'upgrade'
                continue
            
            # ...a jeśli nie, sprawdź czy jest dostępny nowy budynek
            elif locate(self.bot, r"png/build/is_new_building", perform_click=True):
                time.sleep(1)
                # Poniżej poprawiono literówkę w ścieżce z 'builnding' na 'building'
                if locate(self.bot, r"png/build/start_new_building", perform_click=True):
    ...
    def run(self):
        ...
        # ocr_manager.find_text już używa self.bot
        if not self.ocr_manager.find_text("build"):
            # To nie błąd, po prostu wszyscy budowniczy są zajęci.
